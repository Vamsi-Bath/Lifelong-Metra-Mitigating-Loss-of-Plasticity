...
def get_argparser():
    ...
    parser.add_argument('--task_switch_period', type=int, default=0)  # 0 disables switching
    # ==========================================================
    ...
    parser.add_argument('--cbp_rr', type=float, default=1e-4)          # replacement_rate
    parser.add_argument('--cbp_mt', type=int, default=1000)            # maturity_threshold
    parser.add_argument('--cbp_init', type=str, default='kaiming', choices=['default','xavier','lecun','kaiming'])
    parser.add_argument('--cbp_util', type=str, default='contribution',
                        choices=['contribution','weight','random'])    # (CBPLinear uses contribution internally)
    parser.add_argument('--cbp_decay', type=float, default=0.0)        # activation EMA decay in CBPLinear
...
# =====================================================
    ...
    parser.add_argument('--algo', type=str, default='metra', choices=['metra', 'dads', 'ppo'])
...
def get_effective_cp_path(args):
    cp_path = args.cp_path
    if args.mode == "hierarchical" and cp_path is None:
        cp_path = find_latest_policy()
        print("Using latest pretrained policy:", cp_path)
    return cp_path
...
def task_switch_fn(env, task_id):
    ...
    # pick friction scale based on task_id
    scales = [0.5, 1.0, 1.5, 2.0]
    scale = scales[task_id % len(scales)]

    # try common mujoco handles
    model = None
    ...
    # geom_friction[:,0] is sliding friction
...
def task_params_fn(env):
    """
    Return friction/coefficient snapshot for debugging.
    Implement per-env.
    """
...
def get_log_dir():
    ...
    # POSIX-only path length/name limit check
    if hasattr(os, "pathconf"):
        try:
            assert len(exp_name) <= os.pathconf('/', 'PC_NAME_MAX')
        except (OSError, ValueError):
            # Some filesystems don't support this query; ignore
            pass
...
def make_env(args, max_path_length):
    if args.env == 'maze':
        ...
        if args.env == 'dmc_cheetah':
            env = dmc.make('cheetah_run_forward_color', obs_type='states', frame_stack=1, action_repeat=2,
                           seed=args.seed)
            env = RenderWrapper(env)
        elif args.env == 'dmc_quadruped':
            env = dmc.make('quadruped_run_forward_color', obs_type='states', frame_stack=1, action_repeat=2,
                           seed=args.seed)
            env = RenderWrapper(env)
    ...
    cp_path = get_effective_cp_path(args)
...
def run(ctxt=None):
    ...
    effective_cp_path = get_effective_cp_path(args)
    max_path_length = args.max_path_length
    if effective_cp_path is not None:
    ...
    # ----- CBP injection (AFTER nets exist, BEFORE optimizers) -----
    if args.cbp:
        ...
        print("== Enabling CBP ==")
        print({"rr_pol": rr_pol, "rr_q": rr_q, "rr_te": rr_te, "mt_pol": mt_pol, "mt_q": mt_q, "mt_te": mt_te})

        n_pol = add_cbp_to_model(option_policy, **_cbp_kwargs(rr_pol, mt_pol))
        n_te = 0 if traj_encoder is None else add_cbp_to_model(traj_encoder, **_cbp_kwargs(rr_te, mt_te))
        ...
        print("CBP patched sequentials:", dict(policy=n_pol, traj_encoder=n_te, qf1=n_q1, qf2=n_q2, vf=n_vf))

    # ----- Now define optimizers (AFTER CBP patching) -----
    def _finalize_lr(lr):
        if lr is None:
            lr = args.common_lr
        else:
    ...
    if context_encoder is not None:
        optimizers['context'] = torch.optim.Adam(
            ...
            lr=_finalize_lr(args.context_lr),
        )
