...
class OptionLocalRunner(LocalRunner):
    ...
    def resume_train(self, n_epochs=None, batch_size=None, store_paths=None, pause_for_plot=None):
        if not self._has_setup:
            raise NotSetupError('Use setup() or restore() before resume_train().')

        if n_epochs is not None:
            self._train_args.n_epochs = n_epochs
        if batch_size is not None:
            self._train_args.batch_size = batch_size
    ...
    def restore(self, from_dir, make_env, from_epoch='last', post_restore_handler=None):
        ...
        print("RESUMING FROM:", from_dir)
        print("START EPOCH:", self._train_args.start_epoch)
