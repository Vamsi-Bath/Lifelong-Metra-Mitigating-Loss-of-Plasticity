...
class LifelongMETRA(IOD):
    def __init__(
            ...
            *,
            ...
            dual_slack,
            dual_dist,
            task_switch_period,   # kept for compatibility, not used here
    ...
    def _evaluate_policy(self, runner):
        if self.discrete:
            ...
            for i in range(len(random_options)):
                random_option_colors.extend([cm.get_cmap(cmap)(colors[i])[:3]])
            random_option_colors = np.array(random_option_colors)
        else:
            random_options = np.random.randn(
                self.num_random_trajectories,
                self.dim_option,
            )
            if self.unit_length:
                random_options = (
                    random_options
                    / np.linalg.norm(random_options, axis=1, keepdims=True)
                )
        ...
        if self.dim_context > 0 and self.use_context_in_phi:
            ...
            ctx_t = ctx_t.unsqueeze(0).expand(last_obs.shape[0], -1)
            ...
            if self.discrete:
                ...
                if self.dim_option == 2:
                    ...
                    if self.unit_length:
                        video_options = (
                            video_options
                            / np.linalg.norm(video_options, axis=1, keepdims=True)
                        )
                video_options = video_options.repeat(self.num_video_repeats, axis=0)
